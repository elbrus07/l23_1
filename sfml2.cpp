...
public:
    ...
    void rotateLeft(int value) {
        root = applyRotateLeft(root, value);
    }

    void rotateRight(int value) {
        root = applyRotateRight(root, value);
    }
...
private:
    ...
    Node* applyRotateLeft(Node* node, int value) {
        /*Рекурсивный поиск узел со значением value и вызов rotateLeftNode*/
        if (node == nullptr) return nullptr;
        if (node->data == value) {
            if (node->right == nullptr) {
                cout << "Нет правого поддерева\n";
                return node;
            }
            return rotateLeftNode(node);
        }
        if (value < node->data)
            node->left = applyRotateLeft(node->left, value);
        else
            node->right = applyRotateLeft(node->right, value);
        return node;
    }

    Node* applyRotateRight(Node* node, int value) {
        /*Рекурсивный поиск узел со значением value и вызов rotateRightNode*/
        if (node == nullptr) return nullptr;
        if (node->data == value) {
            if (node->left == nullptr) {
                cout << "Нет левого поддерева\n";
                return node;
            }
            return rotateRightNode(node);
        }
        if (value < node->data)
            node->left = applyRotateRight(node->left, value);
        else
            node->right = applyRotateRight(node->right, value);
        return node;
    }
};
